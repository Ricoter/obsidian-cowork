...
export async function migrateCommands() {
  ...
  new ConfirmModal(app, () => {}, message, "🚀 New Cowork Custom Commands", "OK", "").open();
}
...
export async function suggestDefaultCommands(): Promise<void> {
  ...
  if (existingCommands.length === 0) {
    new ConfirmModal(
      ...
      "Would you like to add Cowork recommended commands in your custom prompts folder? These commands will be available through the right-click context menu and slash commands in chat.",
      "Welcome to Cowork",
      "Confirm",
      "Skip"
    ).open();
